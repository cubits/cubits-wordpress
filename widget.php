...
/**
 * Registers the Cubits Button widget
 *
 */

class Cubits_Button extends WP_Widget {
  ...
  public function __construct() {
    //global $wid, $wname;
    parent::__construct(
      'cubits_button', // Base ID
      'Cubits Button', // Name
      array( 'description' => __( 'Displays a Cubits button in your sidebar', 'text_domain' ), ) // Args
    );
    ...
    if($cached !== false) {
      ...
      if( $api_key && $api_secret ) {
        try {
          ...
          $link = $cubits->createInvoice("Alpaca socks", "10.00", "CAD", $options)->invoice_url;
          $button = "<a href='".$link."'>Go to invoice</a>";
        } catch (Exception $e) {
          $msg = $e->getMessage();
          error_log($msg);
          echo "There was an error connecting to Cubits: $msg. Please check your internet connection and API credentials.";
        }
        set_transient($transient_name, $button);
        echo $button;
      } else {
        echo "The Cubits plugin has not been properly set up - please visit the Cubits settings page in your administrator console.x";
      }
    }
  ...
  public function update( $new_instance, $old_instance ) {
    ...
    $instance['price'] = esc_attr(strip_tags( $new_instance['price'] ));
  ...
  public function form( $instance ) {
    ...
    <p>
      ...
      <span>
        <select class="widefat cubits-currency" id="<?php echo $this->get_field_id( 'currency' ); ?>" name="<?php echo $this->get_field_name('price_currency_iso'); ?>">
            <?php
        ...
        <input class="widefat cubits-price" id="<?php echo $this->get_field_id( 'price' ); ?>" name="<?php echo $this->get_field_name( 'price_string' ); ?>" type="text" value="<?php echo esc_attr( $price_string ); ?>" />
      </span>
    </p>
    ...
    <!--<a id="cubits-toggle" class="cubits-toggle">Show Advanced Options</a>-->
    <div id="cubits-advanced" class="cubits-advanced">
      <p>
      ...
      <small>Optional.  This gets passed through in <a target="_blank" href="https://cubits.com/help">callbacks</a> to your site.</small>
      </p>
    </div>
...
// register the widget
add_action( 'widgets_init', create_function( '', "register_widget( 'Cubits_Button' );" ) );